...
def parse_args():
    ...
    # for train
    parser.add_argument('--script', type=str, default='ostrack', choices=['ostrack', 'ugtrack'],
...
if __name__ == "__main__":
    ...
    if args.script == "ostrack":
    ...
    elif args.script == "ugtrack":
        from lib.models.ugtrack import build_ugtrack
        model = build_ugtrack(cfg, training=False)
        seq_len = cfg.DATA.UWB.SEQ_LEN
        search_uwb_seq = torch.randn(bs, seq_len, 2).to(device)
        model = model.to(device)

        macs1, params1 = profile(model, inputs=(search_uwb_seq, 1),
                                 custom_ops=None, verbose=False)
        macs, params = clever_format([macs1, params1], "%.3f")
        print('overall macs is ', macs)
        print('overall params is ', params)
        ...
        with torch.no_grad():
            for i in range(T_w):
                _ = model(search_uwb_seq, stage=1)
            start = time.time()
            for i in range(T_t):
                _ = model(search_uwb_seq, stage=1)
            torch.cuda.synchronize()
