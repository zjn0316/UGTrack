...
from lib.config.ostrack.config import (                          # 导入OSTrack配置相关对象
    cfg,                                                         # 导入OSTrack配置对象
...
def parameters(yaml_name: str):                                  # 根据指定yaml名称构建测试参数
    ...
    # 从yaml文件更新默认配置
    yaml_file = os.path.join(                                    # 拼接OSTrack实验yaml配置文件路径
        prj_dir,                                                 # 使用项目根目录
        'experiments/ostrack/%s.yaml' % yaml_name,               # 使用OSTrack实验配置相对路径
    )                                                            # 结束yaml配置路径拼接
    ...
    # 网络权重文件路径
    params.checkpoint = os.path.join(                            # 拼接OSTrack测试权重路径
        save_dir,                                                # 使用训练输出和权重保存目录
        "checkpoints/train/ostrack/%s/OSTrack_ep%04d.pth.tar" %  # 使用OSTrack权重相对路径模板
