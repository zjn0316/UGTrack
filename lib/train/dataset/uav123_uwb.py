...
class UAV123UWB(BaseVideoDataset):

    def __init__(self, root=None, image_loader=jpeg4py_loader, split='train', seq_ids=None,
                 data_fraction=None, uwb_seq_len=5):

        root = env_settings().uav123_uwb_dir if root is None else root
        super().__init__('UAV123UWB', root, image_loader)
    ...
    def _read_occlusion_anno(self, seq_path):
        ...
        occlusion = pandas.read_csv(occlusion_file, header=None, dtype=np.float32,
    ...
    def _read_uwb_gt_anno(self, seq_path):
        ...
        uwb_gt = pandas.read_csv(uwb_gt_file, delimiter=',', header=None, dtype=np.float32,
                                 na_filter=False, low_memory=False).values
    ...
    def _read_uwb_obs_anno(self, seq_path):
        ...
        uwb_obs = pandas.read_csv(uwb_obs_file, delimiter=',', header=None, dtype=np.float32,
                                  na_filter=False, low_memory=False).values
    ...
    def _read_uwb_conf_anno(self, seq_path):
        ...
        uwb_conf = pandas.read_csv(uwb_conf_file, header=None, dtype=np.float32,
                                   na_filter=False, low_memory=False).values.reshape(-1)
...
UAV123UWB 数据集类主要由训练框架中的采样器 (Sampler) 调用。其核心数据流如下：
1. 采样器通过 `get_sequence_info` 获取整个视频序列的所有标注张量。
2. 采样器根据选定的帧索引 (frame_ids)，调用 `get_frames` 提取对应帧的图像和标注切片。

示例代码 (Example Usage):
--------------------------------------------------
from lib.train.dataset.uav123_uwb import UAV123UWB
from lib.train.data import jpeg4py_loader

# 1. 实例化数据集
# root: 数据集根目录，包含 train/val/test 文件夹
dataset = UAV123UWB(root='/home/zjn/data/UAV123_UWB', split='train', uwb_seq_len=5)
...
# 返回值字段包括: bbox[N,4], valid[N], visible[N], uwb_gt[N,2], uwb_obs[N,2], uwb_seq[N,5,2], uwb_conf[N]
...
# - obj_meta: 包含序列元数据，如 {'object_class_name': 'car1_2', ...}
--------------------------------------------------
"""